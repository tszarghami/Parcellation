function [J,K,VAR] = spm_dcm_J_mod(Y,U,X0,dt,R,I,D)
% This is a minimally MODIFIED version of spm_dcm_J.m in SPM12
% that returns the posterior variances of the connections as well.
...
%--------------------------------------------------------------------------
J     = zeros(nv + nu,nv);
VAR     = zeros(nv + nu,nv); % added
for v = 1:nv
    J(:,v) = Ep{v};
    VAR(:,v) = diag(full(Cp{v})); % added
end
